...
from shared_utils import normalize_business_name, normalize_person_name, normalize_mailing_address, normalize_mailing_address_coarse, first_normalized_mailing_address, is_ignored_shared_address, canonicalize_person_name, get_name_variations
...
def link_properties_standalone(conn):
    ...
    with conn.cursor() as cursor:
        ...
        t0 = time.time()
        logger.info("  [1a] Clearing old property links (business_id, principal_id = NULL)...")
        cursor.execute("UPDATE properties SET business_id = NULL, principal_id = NULL;")
        logger.info(f"  [1a] ✅ Cleared {cursor.rowcount:,} rows in {time.time()-t0:.1f}s")

        # Re-normalize owner fields using updated SQL logic
        t0 = time.time()
        logger.info("  [1b] Re-normalizing owner_norm and co_owner_norm (regex on all rows)...")
        ...
        logger.info(f"  [1b] ✅ Re-normalized {cursor.rowcount:,} rows in {time.time()-t0:.1f}s")

        conn.commit()
        logger.info("  [1] Commit done.")

        # A. Link to Businesses (Check both owner and co_owner)
        logger.info("  [1c] Linking properties to Businesses via owner_norm and co_owner_norm...")
        ...
        logger.info(f"  [1c] Pass 1 (owner_norm): {cursor.rowcount:,} linked in {time.time()-t0:.1f}s")
        ...
        """)
        biz_linked_count += cursor.rowcount
        logger.info(f"  [1c] Pass 2 (co_owner_norm): {cursor.rowcount:,} linked in {time.time()-t0:.1f}s")
        ...
        biz_linked_count += cursor.rowcount
        logger.info(f"  [1c] Pass 3 (suffix-blind): {cursor.rowcount:,} linked in {time.time()-t0:.1f}s")

        conn.commit()
        logger.info(f"  [1c] ✅ Total business links: {biz_linked_count:,}")

        # B. Link to Principals (Check both owner/co_owner and handle permutations)
        t0 = time.time()
        logger.info("  [1d] Linking properties to Principals (where no business was matched)...")
...
def link_orphans_by_address(conn):
    ...
    with conn.cursor(cursor_factory=RealDictCursor) as cursor:
        ...
        for row in cursor.fetchall():
            norm = first_normalized_mailing_address(row['mail_address'], row['business_address'])

            if norm and len(norm) > 10 and norm not in JUNK_ADDRS and not is_ignored_shared_address(norm):
                addr_to_biz[norm].append(row['id'])

        # 2. Keep all normalized non-junk address groups; do not frequency-cap.
        valid_addr_map = {}
        ...
        logger.info(f"indexed {len(valid_addr_map)} valid business addresses.")
...
def build_graph(conn):
    ...
    with conn.cursor(cursor_factory=RealDictCursor) as cursor:
        ...
        # --- NOTE: Manual merges removed — shared-address linking now handles this organically ---
...
def build_address_edges(conn, existing_graph):
    ...
    logger.info(f"Found {len(edges)} hidden edges across {linked_clusters} shared addresses.")
    return edges
